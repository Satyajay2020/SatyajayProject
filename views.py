...
def add_shows(venue_id):
    ...
    if request.method == 'POST':
        ...
        ##vvip
        s = Show(name=name, tags=tags, price=price, show_timing=show_timing, venue=v, show_capacity=show_capacity,
                 simg=simg)
...
def edit_shows(venue_id, show_id):
    ...
    if request.method == 'POST':
        ...
        # db.session.add(s)
        db.session.commit()
        flash(f'Show {show_id} edited successfully to {v.name} at {s.show_timing}!', category="success")
        return redirect(f"/{venue_id}/show")
...
def book_show(user_id, show_id):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(
            f'Congratulations!! You successfully booked {seats} ticket/s for {s.name} at {s.venue.name} at Rs. {total_price}.',
            category="success")

        return redirect(f"/user_dashboard/{u.id}")

    return render_template("book_show.html", admin=current_user, user=current_user, id=current_user.id,
                           show_name=s.name, venue_name=v.name, show_timing=s.show_timing, show_price=s.price,
                           available_s=s.show_capacity, venue_location=v.location)
...
def user_bookings(user_id):
    ...
    if request.method == "POST":
        ...
        if x:  # checking iff x exists in the USR table or not
            ...
            return render_template("user_bookings.html", admin=current_user, user=current_user, id=current_user.id,
                                   ub=user_booking, x=x)
...
def search():
    if request.method == "POST":
        ...
        if op == "Shows":
            ...
            return render_template("user_dashboard.html", user=current_user, admin=current_user, shows=shows,
                                   id=current_user.id)
